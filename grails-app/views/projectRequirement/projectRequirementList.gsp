	...
	<div style="float: right; width: 85%; height: 800px;">
		<g:if test="${session?.isTechincalSupportAccount == false}">
			<div style="margin-top: 20px; text-align:right; margin-bottom: 40px;">
				<g:if test="${projectInfo.status == -1 }">
					<button type="button" class="btn btn-default" onclick="alert('该项目已终止!')">新建需求</button>
				</g:if>
				<g:else>
					<button type="button" class="btn btn-default" onclick="toShowProjectRequirement()">新建需求</button>
				</g:else>
			...
			<table class="table table-bordered">
				<tr>
					...
					<g:each in="${result }" var="requirement">
						<tr>
							<td>
								<g:if test="${requirement.requirement_state==3 || requirement.requirement_state==4}">
									<input type="checkbox" name="requirement_ids" value="${requirement.rId }">
								</g:if>
								...
								<g:else >
									...
									<g:if test="${requirement?.operation_person == session?.operationInfo?.id}">
										<g:if test="${requirement.requirement_state==0}">
											<a href="javascript:editProjectRequirement(${requirement.rId })">编辑</a>
											<a href="javascript:delProjectRequirementInfo(${requirement.rId })">删除</a>
											<g:if test="${requirement.object_type == 10}">
												<a href="javascript:updateProjectRequirementStatus(${requirement.rId },1,'确定要提交此需求吗？')">提交 </a>
											</g:if>
											<g:else>
